...
schemaDeplay = sqlContext.inferSchema(airlineData)
schemaDeplay.registerTempTable("airlineDeplay")

# SQL can be run over SchemaRDDs that have been registered as a table.
deplay = sqlContext.sql("SELECT year, month, carrier, airport, arr_flights, arr_del15 FROM airlineDeplay WHERE arr_del15 > 100")

deplay.printSchema()
    
for line in deplay.collect():
